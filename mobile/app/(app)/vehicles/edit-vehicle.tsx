...
export default function EditVehicleScreen() {
  ...
  const params = useLocalSearchParams<{
    vehicleId: string; make: string; model: string; year: string;
    licensePlate: string; fuelType: string; color?: string; vin?: string;
  }>();
  ...
  const [makeQuery, setMakeQuery] = useState(params.make ?? '');
  const [modelQuery, setModelQuery] = useState(params.model ?? '');
  const [year, setYear] = useState(params.year ?? '');
  const [licensePlate, setLicensePlate] = useState(params.licensePlate ?? '');
  const [fuelType, setFuelType] = useState(params.fuelType ?? 'PETROL');
  ...
  useEffect(() => {
    refApi.makes()
      .then((ms) => {
        setMakes(ms);
        const found = ms.find((m) => m.name.toLowerCase() === params.make?.toLowerCase());
  ...
  const filteredMakes = makeQuery.length > 0 && !selectedMake
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    try {
      await vehiclesApi.update(params.vehicleId, {
        make: makeStr,
        model: modelStr,
        year: yearNum,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Button onPress={handleSubmit} label={t('vehicles.saveVehicle')} loading={saving} size="lg" style={{ marginTop: SPACE[2] }} />
      </ScrollView>
    </KeyboardAvoidingView>
  );
}
...
const s = StyleSheet.create({
  ...
  dropdown: { marginTop: 4, borderWidth: 1, borderRadius: RADIUS.md, overflow: 'hidden' },
